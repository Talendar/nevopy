...
POP_SIZE = 100
IMG_BATCH_SIZE = 5
BATCH_MODE = "avg_inputs"  # "avg_inputs" or "avg_outputs"
LAYERS = [
    ...
    while steps < max_steps:
        ...
        if genome is not None:
            ...
            if len(img_batch) >= IMG_BATCH_SIZE:
                ...
                if BATCH_MODE == "avg_inputs":
                    img_stack = img_stack.sum(axis=0) / IMG_BATCH_SIZE
                    img_stack = np.expand_dims(img_stack, axis=0)

                h = genome.process(img_stack).numpy()
                if BATCH_MODE == "avg_outputs":
                    h = h.sum(axis=0) / IMG_BATCH_SIZE
                else:
                    h = h[0]
...
def new_pop():
    ...
    input_shape = [IMG_BATCH_SIZE, *sample_img.shape]
